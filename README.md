...
[![Game Version](https://img.shields.io/badge/Game-Crimson%20Desert%201.05%E2%80%931.12-D63B3B)](https://www.playcrimsondesert.com/)
...
- **Save read/write is version-agnostic.** Each save embeds its own schema, so
  the editor reads and **re-saves any patch's save (1.05–1.12) in that save's
  ...
  save.
- **Item names / icons resolve against the installed game (currently 1.12).**
  ...
  if an item's key changed between that save's patch and 1.12 it may show as a
  ...
  (right now: anything other than 1.12) — that warning is about your install,
