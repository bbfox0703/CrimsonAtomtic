...
- **Save read/write is version-agnostic.** Each save embeds its own schema, so
  the editor reads and **re-saves any patch's save (1.05–1.11) in that save's
  ...
  save.
- **Item names / icons resolve against the installed game (currently 1.11).**
  ...
  if an item's key changed between that save's patch and 1.11 it may show as a
  ...
  (right now: anything other than 1.11) — that warning is about your install,
