...
(currently **2.03**) and stays compatible with 1.05 / 1.06 saves via schema
...
8. **Vendor deps**: cloned into [`vendor/`](vendor/), never git submodules. `vendor/<name>/` is **read-only** — any change to `crimson-rs` is committed at the source repo `D:\Github\crimson-rs` first, then mirrored in via [`vendor/update_vendors.ps1`](vendor/update_vendors.ps1) (which does `reset --hard origin/main` and silently wipes any local edits). See [`vendor/README.md`](vendor/README.md).
