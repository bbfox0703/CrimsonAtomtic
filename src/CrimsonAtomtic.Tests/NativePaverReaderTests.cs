...
{
    /// <summary>Bit-for-bit copy of the live 1.11.00 install's paver
    /// (<c>01 00 0b 00 00 00 24 7a 2c 20</c> → build 0x202c7a24 LE).</summary>
    private static readonly byte[] Paver_1_11_Live =
        [0x01, 0x00, 0x0b, 0x00, 0x00, 0x00, 0x24, 0x7a, 0x2c, 0x20];

    /// <summary>The previous patch's paver (1.10.00) — kept to pin that
    /// it is now flagged INCOMPATIBLE: 1.11 drifted the iteminfo schema
    /// (crimson-rs <c>8fdeb45</c>, a new per-item <c>u8</c>), so 1.10 is
    /// no longer in <c>CompatibleMinors</c>.</summary>
    private static readonly byte[] Paver_1_10_Prev =
        [0x01, 0x00, 0x0a, 0x00, 0x00, 0x00, 0xac, 0xb2, 0x84, 0xcf];

    [Fact]
    public void TryReadFromBytes_HappyPath_Returns_1_11_Live()
    {
        if (!File.Exists("crimson_rs.dll"))
        {
            return;
        }
        var v = NativePaverReader.TryReadFromBytes(Paver_1_11_Live);
        Assert.NotNull(v);
        Assert.Equal(1, v!.Value.Major);
        Assert.Equal(11, v.Value.Minor);
        Assert.Equal(0, v.Value.Patch);
        Assert.Equal(0x202c7a24u, v.Value.Build);
        Assert.True(v.Value.IsCompatibleWithParser,
            "1.11.00 should be compatible with the current ParserTargetMinor=11");
        Assert.Equal("1.11.00", v.Value.ShortVersionString);
        Assert.Equal("1.11.00 build 0x202c7a24", v.Value.DisplayString);
    }
    ...
    public void TryReadFromBytes_PreviousMinor_FlagsIncompatible()
    {
        ...
        // 1.11 changed the iteminfo layout vs 1.10 (inserted a new per-item
        // boolean u8 unk_post_apply_drop_stat_type — crimson-rs 8fdeb45), so
        // 1.10 is NO LONGER compatible with this parser build
        // (CompatibleMinors = {11}). A user still on 1.10 is warned to update
        // before iteminfo / save-body loading.
        var v = NativePaverReader.TryReadFromBytes(Paver_1_10_Prev);
        Assert.NotNull(v);
        Assert.Equal(10, v!.Value.Minor);
        Assert.False(v.Value.IsCompatibleWithParser,
            "1.10.00 must NOT be compatible — 1.11 drifted the iteminfo schema");
    }
    ...
    public void TryReadFromBytes_LegacyMinor_FlagsIncompatible()
    {
        ...
        // {11} — 1.07 used a different iteminfo layout.
        ...
        Assert.False(v.Value.IsCompatibleWithParser,
            "1.07.xx must NOT be compatible — not in CompatibleMinors {11}");
    }
    ...
    public void TryReadFromBytes_FutureMinor_FlagsIncompatible()
    {
        ...
        // Synthetic 1.12.xx layout: minor = 12 is past the validated set.
        ...
        ReadOnlySpan<byte> bytes =
            [0x01, 0x00, 0x0c, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00];
        var v = NativePaverReader.TryReadFromBytes(bytes);
        Assert.NotNull(v);
        Assert.Equal(12, v!.Value.Minor);
        Assert.False(v.Value.IsCompatibleWithParser,
            "1.12.xx must NOT be compatible — not yet in CompatibleMinors {11}");
    }
    ...
    public void TryReadFromInstall_LiveInstall_PinsCurrent()
    {
        ...
        // be 1.11 forever, and this test should survive a future
