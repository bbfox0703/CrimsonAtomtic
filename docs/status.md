...
>   - **⏸ PARKED follow-on (user's call 2026-05-31): drop the 1.47 MB embed once the dragon is in-game-confirmed.** Plan: add a crimson-rs ABI `insert_list_element_from_bytes(element_hex, type_name_remap[])` (the byte-blob analogue of the existing `transplant_list_element` — same class-name → type-index remap, no source save needed), embed only the ~140-byte `DRAGON_HEX`, and delete `Assets/mounts/dragon_donor.save` + the transplant-from-donor code. Vendor work (edit `D:\Github\crimson-rs` → `update_vendors.ps1` → rebuild dll). Deferred until the user verifies the 187-key fix actually summons (no point cleaning up an unconfirmed feature). Timestamps are NOT a summon gate (the reference's hex ships static `_lastPaidTime`/`_lastBreedingTime` and still summons) — the dragon block was purely knowledge, not a game-time/cooldown issue.
